...
export default function FavoriteReads() {
  return (
    <section
      className="h-[222px] col-span-1 row-span-1 col-start-2 row-start-2 animate-card-reveal [animation-delay:210ms] motion-reduce:animate-none xl:col-span-2 xl:row-span-2 xl:col-start-3 xl:row-start-5 rounded-xl bg-spotify-light-dark overflow-hidden"
      id="favorite-reads"
        ...
        >
          {favoriteReads.map((book) => (
            <li className="group w-16 shrink-0 snap-start" key={book.title}>
              <div className="relative aspect-[2/3] overflow-hidden rounded-md bg-black/20 shadow-lg shadow-black/20 ring-1 ring-white/10 transition-colors md:group-hover:ring-white/25">
                <Image
                  src={book.cover}
              ...
              <p className="mt-2 line-clamp-2 text-xs font-semibold leading-4 text-spotify-white transition-colors md:group-hover:text-spotify-green">
                {book.title}
              </p>
            </li>
          ))}
